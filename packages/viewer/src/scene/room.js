...
import {
  makeRugTexture, makeWallTexture, makePanelTexture, makeSconceTexture,
} from "./textures.js";
...
function buildWallRun(parent, axis, at, segments, inwardNormal, wallMat, panelMat, trimMat) {
  ...
  segments.forEach(function (seg) {
    ...
    const upper = new THREE.Mesh(new THREE.PlaneGeometry(len, ceiling - wainTop), wallMat);
    const wain = new THREE.Mesh(new THREE.PlaneGeometry(len, wainTop), panelMat);
...
function buildPilasters(parent, axis, at, start, end, inwardNormal, mat, skip) {
  ...
  for (let i = 0; i <= count; i++) {
    ...
    if (skip && p > skip.min && p < skip.max) continue;
    positions.push(p);
  }
...
function buildCeiling(parent, bounds, beamMat) {
  ...
  const base = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({
      color: 0x241a10, roughness: 0.96, emissive: 0x1c1206, emissiveIntensity: 1.0,
    }));
...
function buildSconce(parent, x, z, faceYaw, sconceTex, brassMat) {
  ...
  const back = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.22, 0.06), brassMat);
  group.add(back);
...
function buildClock(parent, x, y, z, faceYaw, brassMat) {
  const group = new THREE.Group();
  const rim = new THREE.Mesh(new THREE.TorusGeometry(0.3, 0.035, 10, 28), brassMat);
  group.add(rim);
  ...
  const tickMat = new THREE.MeshStandardMaterial({ color: 0xb59a68, roughness: 0.5, metalness: 0.5 });
  for (let i = 0; i < 12; i++) {
  ...
  const handMat = new THREE.MeshStandardMaterial({ color: 0xd8bd7f, roughness: 0.4, metalness: 0.7 });
  function makeHand(width, len, zoff) {
    const pivot = new THREE.Group();
    const bar = new THREE.Mesh(new THREE.BoxGeometry(width, len, 0.01), handMat);
  ...
  const hub = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.02, 12), brassMat);
...
export function buildRoom(theme, scene, floorTex, woodTex, bounds, doorway) {
  ...
  const wallMat = new THREE.MeshStandardMaterial({
    map: wallTex, color: theme.plaster.tint, roughness: 0.92, metalness: 0.0,
    emissive: theme.plaster.emissive, emissiveIntensity: 1.0, side: THREE.FrontSide,
  });
  const panelMat = panelMaterial(panelTex);
  panelMat.emissive = new THREE.Color(theme.wainscot.emissive);
  ...
  const brassMat = new THREE.MeshStandardMaterial({
    color: theme.metal.brass, roughness: 0.4, metalness: 0.8,
  });

  floorTex.repeat.set(Math.round(length / 3.5), Math.round(depth / 2.6));
  const floor = new THREE.Mesh(
    new THREE.PlaneGeometry(length, depth),
    new THREE.MeshStandardMaterial({ map: floorTex, roughness: 0.32, metalness: 0.0 }));
  ...
  const rug = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ map: makeRugTexture(theme), roughness: 0.94, metalness: 0.0 }));
  ...
  buildWallRun(root, "x", bounds.zMax, entrySegments, -1, wallMat, panelMat, trimMat);
  buildPilasters(root, "x", bounds.zMax, bounds.xMin, bounds.xMax, -1, panelMat, entryPilasterSkip);

  if (doorway) {
    const lintel = new THREE.Mesh(
      new THREE.PlaneGeometry(doorway.width, ceiling - doorway.height), wallMat);
    lintel.rotation.y = Math.PI;
    lintel.position.set(doorway.x, (doorway.height + ceiling) / 2, bounds.zMax);
    root.add(lintel);
  ...
  const updateClock = buildClock(root, clockX, 2.9, bounds.zMax - 0.05, Math.PI, brassMat);
  ...
  const entryFill = new THREE.PointLight(theme.light.fillColor, 7.0, 12, 2);
  entryFill.position.set(xMid, 2.7, bounds.zMax - 1.6);
  root.add(entryFill);
  const endFill = new THREE.PointLight(theme.light.fillColor, 4.6, 8, 2);
  endFill.position.set(bounds.xMin + 1.3, 2.4, STACKS.walkwayDepth / 2 + 0.3);
  const endFill2 = new THREE.PointLight(theme.light.fillColor, 4.6, 8, 2);
  ...
  const wallThick = 0.5;
  const colliders = [
    { minX: bounds.xMin - wallThick, maxX: bounds.xMin, minZ: bounds.zMin - 1, maxZ: bounds.zMax + 1 },
    { minX: bounds.xMax, maxX: bounds.xMax + wallThick, minZ: bounds.zMin - 1, maxZ: bounds.zMax + 1 },
    { minX: bounds.xMin - 1, maxX: bounds.xMax + 1, minZ: bounds.zMin - wallThick, maxZ: bounds.zMin },
  ];
  if (doorway) {
    const dLeft = doorway.x - doorway.width / 2;
    const dRight = doorway.x + doorway.width / 2;
    colliders.push(
      { minX: bounds.xMin - 1, maxX: dLeft, minZ: bounds.zMax, maxZ: bounds.zMax + wallThick },
      { minX: dRight, maxX: bounds.xMax + 1, minZ: bounds.zMax, maxZ: bounds.zMax + wallThick });
  } else {
    colliders.push({ minX: bounds.xMin - 1, maxX: bounds.xMax + 1, minZ: bounds.zMax, maxZ: bounds.zMax + wallThick });
  }

  return { colliders: colliders, updateClock: updateClock, root: root };
}
